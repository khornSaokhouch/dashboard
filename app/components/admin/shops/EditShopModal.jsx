...
export default function EditShopModal({ isOpen, onClose, onSubmit, isUpdating, editingShop, translations }) {
    ...
    if (isOpen) {
        ...
        if (editingShop) {
            setFormData({
                ...
                owner_user_id: editingShop.owner_user_id || "",
                open_time: editingShop.open_time || "08:00",
                close_time: editingShop.close_time || "17:00",
                latitude: editingShop.latitude || 11.5564,
                longitude: editingShop.longitude || 104.9282,
                image: null, 
            });
            setImagePreview(editingShop.image_url || null);
            const initialPosition = L.latLng(editingShop.latitude || 11.5564, editingShop.longitude || 104.9282);
  ...
  }, [isOpen, editingShop, fetchAllUsers]);
    ...
    <Transition appear show={isOpen} as="div">
      <Dialog as="div" className="relative z-[9999]" onClose={onClose}>
        ...
        <div className="fixed inset-0 overflow-hidden flex items-center justify-center p-4">
          ...
          >
             {/* Wider Modal (max-w-5xl) for split layout */}
             <Dialog.Panel className="w-full max-w-5xl transform overflow-hidden rounded-2xl bg-white shadow-2xl transition-all flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50 shrink-0">
                <div>
                  <Dialog.Title as="h3" className="text-lg font-bold text-gray-900">
                    {translations.editShop || "Edit Shop"}
                  </Dialog.Title>
                  <p className="text-xs text-gray-500">Update information and location.</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-hidden flex flex-col">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 h-full">
                    ...
                    <div className="lg:col-span-5 p-6 space-y-5 overflow-y-auto custom-scrollbar">
                      ...
                      <div className="flex items-center gap-4 p-3 border border-dashed border-gray-300 rounded-xl bg-gray-50/50 hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 truncate">Max 10MB. Leave blank to keep current.</p>
                    ...
                    <div className="lg:col-span-7 relative h-96 lg:h-auto bg-gray-100 border-l border-gray-200">
                      ...
                      <MapContainer
                        ...
                        key={formData.id} // Re-render map on change
                        className="z-0"
                      >
                        <TileLayer
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3 shrink-0">
                  ...
                  <button type="submit" disabled={isUpdating} className="inline-flex items-center gap-2 px-6 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-lg shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed">
                    {isUpdating ? <ArrowPathIcon className="w-4 h-4 animate-spin" /> : <PencilSquareIcon className="w-4 h-4" />}
                    {translations.updateShop || "Update Shop"}
                  </button>
                </div>
              </form>
